...
router.route('/')
  .get((request, response) =>
    Promise.all([
      gamesClient.fetch({ state: 'pending', limit: 3, order: 'asc' }, request.rid),
      gamesClient.fetch({ state: 'final', limit: 3, order: 'desc' }, request.rid),
...
router.route('/games')
  .post((request, response) => gamesClient.create(request.session.playerId, request.rid)
    .then(result => response.redirect(`/games/${result.body.id}`)));

router.param('game_id', async (request, response, next, id) => {
  const timer = logger.startTimer();
  const result = await gamesClient.get(id, request.rid);
  request.game = result.body;
  timer.done(`${request.rid} getGameById #${id}`);
  next();
});
...
router.route('/games/:game_id')
  .get(async (request, response, next) => {
    ...
    const { body: rules } = await gamesClient.rules(request.rid);
...
router.route('/games/:game_id/choice')
  .post(async (request, response) => {
    ...
    const result = await gamesClient.update(request.game.id, body, request.rid);
...
router.route('/games/:game_id/judge')
  .post(async (request, response) => {
    const result = await gamesClient.judge(request.game.id, request.rid);
